...
class DockingPath():
    def __init__(self, mod_dock, dock_dock, mod_docker, dock_docker, duration):
        ...
        loc_offset = absolute_xyz(np.array([0,0,0]), mod_docker.equipment[dock_docker][0], mod_docker.orientation, mod_docker.size)
        ...
        self.c0 = UnivariateSpline(x, np.array([self.start_coords[0], self.start_coords[0]+10*math.cos(self.start_orient[0]), self.end_coords[0]-30*math.sin(self.end_orient[0]), self.end_coords[0]]), w=w,k=1, s=0)
        self.c1 = UnivariateSpline(x, np.array([self.start_coords[1], self.start_coords[1]+10*math.sin(self.start_orient[1]), self.end_coords[1]-30*math.cos(self.end_orient[1]), self.end_coords[1]]), w=w,k=1, s=0)
        self.c2 = UnivariateSpline(x, np.array([self.start_coords[2], self.start_coords[2], self.end_coords[2], self.end_coords[2]]), w=w,k=1, s=0)
            
        self.o0 = UnivariateSpline(x, np.array([self.start_orient[0], self.start_orient[0], self.end_orient[0], self.end_orient[0]]), w=w,k=1, s=0)
        self.o1 = UnivariateSpline(x, np.array([0,0,0,0]), w=w,k=1, s=0)
